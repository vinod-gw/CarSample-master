...
public class LogInHandler extends AbstractActionHandler<LogInAction, LogInResult> {
    ...
    public LogInResult execute(LogInAction action, ExecutionContext context) throws ActionException {
        ...
        else if(action.getAccountid().equals("carsample")) {
            user = getUserFromCredentials(action.getUsername(), action.getPassword());
        }
        else{
            user = getUserFromCredentials(action.getUsername(), action.getPassword(), action.getAccountid());
        }
    ...
    private User getUserFromCredentials(String username, String password, String accountid) {
    	System.out.println("accntttt login handler"+accountid);
    	User user = null;
        try {
            user = authenticator.authenticateCredentials(username, password, accountid);
